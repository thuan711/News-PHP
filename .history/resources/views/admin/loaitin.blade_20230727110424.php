            ...
            <tr><td class="align-middle"> {{$row->id}} </td>
                ...
                <td class="align-middle text-end">               
                    <form action="/admin/loaitin/{{$row->id}}" method="post">
                        <a href="/admin/loaitin/{{$row->id}}/edit" class="edit-link"><i class="fa-regular fa-pen-to-square"></i> <span class="tooltip">Sửa</span></a>            
                        <button onclick="return confirm('Bạn muốn xóa danh mục {{$row->ten}}?')" type="submit" class="edit-link"><i class="fa-solid fa-trash"></i> <span class="tooltip">Xóa</span></button>
                        @csrf  @method('DELETE')
                    </form>
                </td>
            </tr>
